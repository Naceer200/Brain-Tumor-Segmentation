...
def save_segmented_slices(images, output = "resultss"):
    # Create the output folder if it doesn't exist
    os.makedirs(output, exist_ok=True)
    filename = os.path.join(output, f"testing.png")
...
if __name__ == "__main__":
    ...
    for x, y in tqdm(zip(test_x, test_y), total=len(test_y)):
        ...
        cat_images = np.concatenate([image, line, mask, line, y_pred], axis=1)
        save_image_path = os.path.join("resultss", name)
        save_segmented_slices(cat_images,save_image_path)
        #cv2.imwrite(save_image_path, y_pred)
